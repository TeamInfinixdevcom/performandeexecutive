...
console.log('DEBUG firebase-functions type:', typeof functions);
try{ console.log('DEBUG firebase-functions keys:', Object.keys(functions)); }catch(e){ console.log('DEBUG firebase-functions keys error', e && e.message); }
console.log('DEBUG firebase-functions.firestore type:', typeof (functions && functions.firestore));
try{ console.log('DEBUG firebase-functions.firestore keys:', Object.keys(functions.firestore)); }catch(e){ console.log('DEBUG firestore keys error', e && e.message); }
try{ console.log('DEBUG firebase-functions.identity keys:', Object.keys(functions.identity || {})); }catch(e){ console.log('DEBUG identity keys error', e && e.message); }
...
// ============================================
// Usar nueva API de triggers de Firestore
exports.auditUserChanges = functions.firestore.onDocumentUpdated('users/{userId}', async (change, context) => {
...
exports.createUserDocument = functions.identity.user().onCreate(async (user) => {
  try {
    const userData = {
      email: user.email,
...
exports.onUserDeleted = functions.identity.user().onDelete(async (user) => {
  try {
...
exports.onVentaUpdated = functions.firestore.onDocumentUpdated('ventas/{id}', async (change, context) => {
  return processVentaEntregado(change, context, 'ventas');
});

exports.onVentaHogarUpdated = functions.firestore.onDocumentUpdated('ventas_hogar/{id}', async (change, context) => {
